...
class ModularShifts(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        x_recon = F.leaky_relu(x_recon.view(batch_size, length, self.input_dim))
        x_recon = torch.linalg.solve((check_tensor(torch.eye(self.input_dim))[None, None, :, :] - B).repeat(batch_size, 3, 1, 1), x_recon[:, :, :, None])
