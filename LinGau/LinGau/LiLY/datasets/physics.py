...
from LiLY.tools.physics_engine import BallEngine

from LiLY.tools.utils import rand_int
...
class PhysicsDatasetPCL(Dataset):
    ...
    def __init__(self, args, phase='raw', trans_to_tensor=None, loader=default_loader):
        ...
        dataf = os.path.join(args.ROOT, args.DATASET)
        self.dataf = dataf
    ...
    def retrieve_by_index(self, idx):
        ...
        imgs = []
        arrs = []
        data_path = os.path.join(self.dataf, self.phase, str(src_rollout) + '.h5')
        metadata = load_data(self.data_names, data_path)
        # load images for dynamics prediction
        if self.load_img:
            for i in range(self.length):
                path = os.path.join(self.dataf, self.phase, str(src_rollout), 'fig_%d%s' % (src_timestep + i, suffix))
