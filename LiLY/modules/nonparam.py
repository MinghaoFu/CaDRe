...
from .components.transition import NPChangeTransitionPrior, NPTransitionPrior, NPStatePrior, NPMaskChangeTransitionPrior
...
class ModularShifts(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        loss = s_recon_loss + self.beta * past_kld_dyn + self.gamma * future_kld_dyn + \
            self.sigma * kld_obs + 1e-2 * s_kld_dyn + 1e-3 * spr_loss + 1e-3 * ind_loss + 1e-3 * ins_spa
