...
import sys
sys.path.append('..')
from LiLY.modules.modularB import ModularShiftsVaryB
from LiLY.tools.utils import load_yaml, setup_seed
from LiLY.datasets.sim_dataset import VaryBDataset
...
def main(args):
    ...
    data = VaryBDataset(directory=cfg['ROOT'],
    ...
    if cfg['LOAD_CHECKPOINT']:
        model = ModularShiftsVaryB.load_from_checkpoint(checkpoint_path=cfg['CHECKPOINT'],
        ...
        model = ModularShiftsVaryB(input_dim=cfg['VAE']['INPUT_DIM'],
                            length=cfg['VAE']['LENGTH'],
                            obs_dim=cfg['SPLINE']['OBS_DIM'],
                            dyn_dim=cfg['VAE']['DYN_DIM'],
                            lag=cfg['VAE']['LAG'],
                            ...
                            decoder_dist=cfg['VAE']['DEC']['DIST'],
                            correlation=cfg['MCC']['CORR'],
                            graph_thres=cfg['B']['GRAPH_THRES'],
                            )

    log_dir = os.path.join(cfg["LOG"], current_user, args.exp)

    checkpoint_callback = ModelCheckpoint(monitor='train_mcc', 
                                          save_top_k=1, 
                                          mode='max')

    early_stop_callback = EarlyStopping(monitor="train_mcc", 
                                        min_delta=0.00, 
                                        patience=50, 
                                        verbose=False, 
                                        mode="max")
    ...
    trainer = pl.Trainer(default_root_dir=log_dir,
                         ...
                         gradient_clip_val=cfg['VAE']['GRAD_CLIP']
                         )
    # model = model.to('cuda')
    # model.reconstruct.pretrain_sparse(train_loader, 1)
    # model.disentangle.pretrain_sparse(train_loader, 1)
    # Train the model
